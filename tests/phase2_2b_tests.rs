use pendulum_kelly_cli::api::MockMarketProvider;
use pendulum_kelly_cli::engine::valuation::calculate_proxy_valuations;
use pendulum_kelly_cli::models::{
    AssetConfig, AssetHolding, ConfigRoot, MarketConfig, PortfolioConfig, PortfolioState,
};
...
fn test_proxy_valuation_calculation() {
    ...
    let results = calculate_proxy_valuations(&config, &state, &provider);
