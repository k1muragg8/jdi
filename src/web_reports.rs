...
pub async fn build_daily_report(
    ...
    let settlements: Vec<models::DcaSettlement> = state.repo.load_settlements(&ctx).await.unwrap_or_default();
    let snapshots: Vec<models::AlipaySnapshot> = state
        .repo
        .load_alipay_snapshots(&ctx)
        .await
        .unwrap_or_default();
    let transactions: Vec<models::Transaction> = state.repo.load_transactions(&ctx).await.unwrap_or_default();
    ...
    for asset in &config.assets {
        if let Some(s) = latest_snaps.get(&asset.asset_id) {
            reconciliation_results
                .push(engine::reconciliation::reconcile_asset(&config, &p_state, s));
        }
    }
...
pub async fn api_reports_weekly_handler(
    ...
    let target_end = params
        .end
        .unwrap_or_else(|| Local::now().format("%Y-%m-%d").to_string());
    let target_start = params.start.unwrap_or_else(|| {
