...
pub fn generate_buy_suggestions(
    ...
    for sec in allocation_sectors {
        let proportion = sec.score / total_score;
        let mut allocated = daily_budget * proportion;

        if allocated > sec.gap {
            allocated = sec.gap;
        }
        if max_single_sector > 0.0 && allocated > max_single_sector {
            allocated = max_single_sector;
        }
        ...
        let enabled_assets: Vec<_> = config
            ...
            .collect();
        if enabled_assets.is_empty() || allocated <= 0.0 {
            continue;
        }

        let asset_count = enabled_assets.len() as f64;
        // TODO: Implement inverse market value distribution. For now, equal weight.
        let per_asset_allocated = allocated / asset_count;

        let mut asset_suggestions = Vec::new();
        let mut actual_sector_allocated = 0.0;

        for asset in enabled_assets {
            let mut final_asset_buy = per_asset_allocated;
            if max_single_asset > 0.0 && final_asset_buy > max_single_asset {
                final_asset_buy = max_single_asset;
            }

            // Cap asset buy at remaining sector gap (mostly redundant with equal weight, but requested)
            if final_asset_buy > sec.gap {
                final_asset_buy = sec.gap;
            }
