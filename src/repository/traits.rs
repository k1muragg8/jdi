...
pub trait PortfolioRepository: Send + Sync {
    ...
    async fn save_transactions(&self, ctx: &RepositoryContext, transactions: &[Transaction]) -> Result<()>;
}
...
pub trait DcaRepository: Send + Sync {
    ...
    async fn save_settlements(&self, ctx: &RepositoryContext, settlements: &[DcaSettlement]) -> Result<()>;
}
...
pub trait ReconciliationRepository: Send + Sync {
    ...
    async fn save_alipay_snapshots(&self, ctx: &RepositoryContext, snapshots: &[AlipaySnapshot]) -> Result<()>;
    async fn load_reconciliation_audits(&self, ctx: &RepositoryContext) -> Result<Vec<ReconciliationAudit>>;
    async fn save_reconciliation_audits(&self, ctx: &RepositoryContext, audits: &[ReconciliationAudit]) -> Result<()>;
}

#[async_trait]
pub trait InstrumentRepository: Send + Sync {
    async fn load_instruments(&self, ctx: &RepositoryContext) -> Result<Vec<InstrumentConfig>>;
    async fn save_instruments(&self, ctx: &RepositoryContext, instruments: &[InstrumentConfig]) -> Result<()>;
    async fn load_instrument_cache(&self, ctx: &RepositoryContext) -> Result<InstrumentQuoteCache>;
    async fn save_instrument_cache(&self, ctx: &RepositoryContext, cache: &InstrumentQuoteCache) -> Result<()>;
}

#[async_trait]
pub trait ReportRepository: Send + Sync {
    async fn load_portfolio_snapshots(&self, ctx: &RepositoryContext) -> Result<Vec<PortfolioSnapshot>>;
    async fn save_portfolio_snapshots(&self, ctx: &RepositoryContext, snapshots: &[PortfolioSnapshot]) -> Result<()>;
    async fn save_markdown_report(&self, ctx: &RepositoryContext, content: &str, filename: &str) -> Result<()>;
}

#[async_trait]
pub trait AuditRepository: Send + Sync {
    async fn load_web_admin_audit(&self, ctx: &RepositoryContext) -> Result<WebAdminAuditLog>;
    async fn append_web_admin_audit(&self, ctx: &RepositoryContext, record: WebAdminAudit) -> Result<()>;
}

#[async_trait]
pub trait CacheRepository: Send + Sync {
    async fn load_cache_status(&self, ctx: &RepositoryContext) -> Result<CacheStatusRegistry>;
    async fn save_cache_status(&self, ctx: &RepositoryContext, registry: &CacheStatusRegistry) -> Result<()>;
    ...
    async fn save_proxy_cache(&self, ctx: &RepositoryContext, cache: &ProxyValuationCache) -> Result<()>;
